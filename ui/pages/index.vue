    ...
    <div class="grid grid-cols-12 gap-4">
      <section class="col-span-12 lg:col-span-9">
        ...
        <div class="grid gap-3 items-start grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          <nuxt-link v-for="(l, li) in favourites" :key="li"
            class="p-4 h-fit card card-bordered bg-base-200 hover:bg-base-300 transition-colors"
            :to="'/registry/' + l.slug">
            <span class="text-lg flex items-center">
              <Icon size="40" :name="l.icon" />&emsp;{{ l.name }}
            </span>
          </nuxt-link>
        </div>
        <!-- Navigation Links -->
        <div class="space-y-4" v-for="(l, li) in links" :key="li">
          ...
          <div class="grid gap-3 items-start grid-cols-1 sm:grid-cols-2">
            <nuxt-link v-for="(l, li) in l.children" :key="li"
              class="p-4 h-fit card card-bordered bg-base-200 hover:bg-base-300 transition-colors" :to="l.url">
              <span class="text-lg flex items-center">
                <Icon size="40" :name="l.icon" />&emsp;{{ l.name }}
              </span>
            </nuxt-link>
      ...
      <aside class="col-span-12 lg:col-span-3 card card-bordered bg-base-100 space-y-3" v-if="stats">
        ...
        <div class="p-3 card">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-xs text-base-content/70">{{ (averageCpu || 0).toFixed(1) }}%</span>
          </div>
          <!-- CPU Core Grid -->
          <div class="grid grid-cols-4 gap-1">
            <div v-for="(usage, index) in stats.cpu" :key="index"
              class="min-h-6 rounded-xl flex items-center justify-center text-[10px] font-bold transition-all duration-300"
              :class="getCpuCoreClass(usage)" :title="`Core ${index + 1}: ${(usage || 0).toFixed(1)}%`">
              {{ index + 1 }}
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center">
              <div :style="{ color: getUsageBackground(stats.memory) }">
                <span class="text-6xl italic font-black">{{ (stats.memory || 0).toFixed(1) }}</span>&nbsp;<span
                  class="text-xs text-base-content/70">% RAM used</span>
              </div>
            </div>
          </div>
          <!-- Memory Progress Bar -->
          <div class="w-full rounded-full min-h-1 bg-black overflow-hidden">
            <div class="h-full transition-all min-h-1 duration-500 rounded-full"
              :style="{ width: '100%', background: getUsageBackground(stats.memory) }"></div>
          </div>
        </div>
        ...
        <div class="p-3 card" v-if="stats.disk_usage !== undefined">
          ...
          <div class="w-full bg-black text-white rounded-sm overflow-hidden">
            <div class="h-full px-2 flex items-center justify-between transition-all duration-500"
              :style="{ width: '100%', background: getUsageBackground(stats.disk_usage) }">
              <span class="text-sm font-medium flex items-center">
                <Icon name="mdi:harddisk" size="16" class="mr-1" /> Disk
              </span>
              <span class="text-xs">{{ (stats.disk_usage || 0).toFixed(1) }}%</span>
            </div>
          </div>
        </div>
        ...
        <!-- System Info -->
        <div
          class="p-4 card bg-gradient-to-br from-base-100 to-base-200 border border-base-300 shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            <div class="text-center p-1 rounded-lg bg-base-200/30 hover:bg-base-200/50 transition-all">
              <div class="text-4xl font-bold text-content-base">{{ ((stats.total_memory || 0) / 1024).toFixed(1) }}
              </div>
            ...
            <!-- Storage -->
            <div class="text-center p-1 rounded-lg bg-base-200/30 hover:bg-base-200/50 transition-all">
              <div class="text-4xl font-bold text-content-base">{{ ((stats.disk_total || 0) / 1024).toFixed(1) }}</div>
...
function getStats() {
  ...
  statsConnection = useNuxtApp().$sseConnect(
    "stats",
    (dataStr) => {
      try {
        stats.value = JSON.parse(dataStr);
      } catch (e) {
        console.error("Error parsing stats data:", e);
      }
    },
    (err) => {
      console.error("Error with stats SSE connection:", err);
    }
  );
}
...
function getUsageBackground(usage) {
  ...
  return `linear-gradient(90deg, ${color} 0%, transparent ${Math.ceil(usage || 0).toFixed()}%)`;
}
